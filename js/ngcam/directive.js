...
(function(angular) {
    ...
    directive.$inject = ['$q', '$timeout','$cookieStore'];

    function directive($q, $timeout,$cookieStore) {
        return {
            'restrict': 'E',
            'scope': {
                'actionMessage': '@',
            ...
            'template': ['<div class="ng-camera">',
                ...
                '</div>',
                '<button ng-hide="true" id="ng-camera-action" ng-click="getSnapshot()">{{actionMessage}}</button>',
                '</div>'].join(''),
            'link': link
        };
